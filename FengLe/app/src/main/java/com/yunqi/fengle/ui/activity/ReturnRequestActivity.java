...
import com.yunqi.fengle.ui.adapter.ReturnTableDataAdapter;
 ...
 * 退货申请
 */
public class ReturnRequestActivity extends BaseActivity<ReturnQueryPresenter> implements ReturnRequestContract.View, RadioGroup.OnCheckedChangeListener {
    ...
    protected void initEventAndData() {
        ...
        adapter = new ReturnTableDataAdapter(this, mlistReturnApply);
    ...
    private void setWidgetListener() {
        tableViewEx.tableView.addDataClickListener(new TableDataClickListener<ReturnApply>() {
            @Override
            public void onDataClicked(int rowIndex, ReturnApply ReturnApply) {
                Intent intent = new Intent(ReturnRequestActivity.this, ReturnDetailsActivity.class);
                intent.putExtra("position", rowIndex);
                intent.putExtra("status", mStatus);
                intent.putExtra("ReturnApply", ReturnApply);
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ...
                        if (lstartTime > 0 && lendTime > 0 && lstartTime >= lendTime) {
                            ToastUtil.showNoticeToast(ReturnRequestActivity.this, getString(R.string.warming_time_select));
                            return;
                        }
        ...
        RxView.clicks(btnStartTime)
                ...
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
                        TimeSelectDialog dialog = new TimeSelectDialog(ReturnRequestActivity.this, new TimeSelectDialog.TimeSelectListener() {
                            @Override
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ADD_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            mlistReturnApply.remove(position);
    ...
    @Override
    public void showContent(List<ReturnApply> listReturnApply) {
        if (listReturnApply.isEmpty()) {
            Log.w(TAG, "No data!");
            tableViewEx.setEmptyData();
            mlistReturnApply.clear();
            adapter.notifyDataSetChanged();
            return;
        }
        tableViewEx.setLoadMoreEnabled(true);
        mlistReturnApply.clear();
        mlistReturnApply.addAll(listReturnApply);
        adapter.notifyDataSetChanged();
    }

    @Override
    public void showMoreContent(List<ReturnApply> listReturnApplyMore) {
        if (listReturnApplyMore.isEmpty()) {
            tableViewEx.setLoadMoreEnabled(false);
            Log.w(TAG, "No more data!");
            return;
        }
        tableViewEx.setLoadMoreEnabled(true);
        mlistReturnApply.addAll(listReturnApplyMore);
        adapter.notifyDataSetChanged();
    }
}
